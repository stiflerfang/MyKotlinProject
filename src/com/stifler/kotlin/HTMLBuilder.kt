...
open class TagOfTable(val name:String){
    private val children = mutableListOf<TagOfTable>()

    protected fun <T:TagOfTable> doInit(child:T,init:T.() -> Unit){
        child.init()
        children.add(child)
    }
...
class TABLE : TagOfTable("table"){
    fun tr(init:TR.() -> Unit) = doInit(TR(),init)
}

class TR : TagOfTable("tr"){
    fun td(init:TD.() -> Unit)= doInit(TD(),init)
}

class TD : TagOfTable("td")
